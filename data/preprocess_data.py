...
def build_dictionary_wordnet(filepaths, dst_path=None, lowercase=False, remove_phrase=True):
    ...
    print('number of phrases: {}'.format(count_phrase))
    print('size of word dictionary: {}'.format(len(word_id_num)))
    print('size of synset_id dictionary: {}'.format(len(id_word)))
    print('size of synset_id_num dictionary: {}'.format(len(id_num_word)))
...
def features2pkl_binary(feat_path, out_path):
    bk_for_x = {}
    np.random.seed(1234)
    ...
    print('feature2pkl_binary size', len(bk_for_x.keys()), count)

def features2pkl_continuous(feat_path, out_path):
    bk_for_x = {}
    np.random.seed(1234)
    ...
    print('feature2pkl_contious size', len(bk_for_x.keys()), count)
...
def CoreNLP(file_path):
    if not os.path.exists('tokenize_and_lemmatize.class'):
        print('Compile ...')
        cmd = 'javac -cp "./corenlp/stanford-corenlp-full-2018-10-05/*" tokenize_and_lemmatize.java'
        print(cmd)
        os.system(cmd)
    print('Run ...\n')
    ...
    cmd = 'java -cp ".:./corenlp/stanford-corenlp-full-2018-10-05/*" tokenize_and_lemmatize {} {} {}'.format(file_path, out_path_token, out_path_lemma )
    print(cmd)
    os.system(cmd)
