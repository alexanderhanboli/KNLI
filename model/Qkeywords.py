...
class Sim(nn.Module):
    ...
    def forward(self, x, y, alpha):
        ...
        x: [B, T, D]
        '''
        s0 = self.cos(x, y).unsqueeze(2) # [B, T, 1]

        input = torch.cat((x, y, x-y, x.mul(y)), -1) # [B, T, 4*D]
        s1 = self.simf(input) # [B, T, 1]
...
#### STEM model
##############################
class Qkeywords(nn.Module):

    def __init__(self, hidden_size = 512, drop_rate = 0.1,
                 num_layers = 4, heads = 4, embd_dim = 300, word_embd_dim = 300):
        '''
    ...
    def forward(self, question, answer, qmask=None, amask=None, sharpening=True, thrd=0.10, alpha=0.25):
        '''
         input : batch, seq_len
                qxlen [qx, lenx]
        ...
        q_cossim = self.sim(question, question_align, alpha) # [B, T, 1]
        ...
        if qmask is not None:
            ...
            q_weight = F.softmax(q_uweight, dim=1) # [B, T, 3]
