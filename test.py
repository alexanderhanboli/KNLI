...
def evaluate(data, params, use_mask = True, print_out = False):
    ...
    if args.model_name.lower() == 'semultitask':
        ...
        for qa in q_attn_list:
            loss_eval = loss_eval + params.multitask_scale * 10 * LL_qa(qa[:,:params.num_concepts,:,:].float(), concept_qa.float())
        for aq in a_attn_list:
            loss_eval = loss_eval + params.multitask_scale * 10 * LL_aq(aq[:,:params.num_concepts,:,:].float(), concept_aq.float())
    else:
