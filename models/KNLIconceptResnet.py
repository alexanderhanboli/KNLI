...
def concept_attention(query, key, value, hQ, hK, hV, mask=None, dropout=None):
    '''
     ...
     hK, hV: [B, H, T2, T1, d_k]
    '''
    b_size, heads, T1, T2, d_k = hQ.size()
    ...
    # (q + hL) * (k + hR)
    q = query.unsqueeze(3) # [B, H, T1, 1, d_k]
    q_plus_hQ = q + hQ # [B, H, T1, T2, d_k]
    k = key.unsqueeze(3) # [B, H, T2, 1, d_k]
    k_plus_hK = (k + hK).transpose(2, 3) # [B, H, T1, T2, d_k]
    scores = torch.sum(q_plus_hQ * k_plus_hK, dim=-1, keepdim=False) / math.sqrt(d_k)# [B, H, T1, T2]
...
class ConceptMultiHeadAttn(nn.Module):

    def __init__(self, heads=4, d_model=300, dropout=0.1):
        "Take in number of heads, model size, and dropout rate."
        super(ConceptMultiHeadAttn, self).__init__()
        assert d_model % heads == 0
        # We assume d_v always equals d_k
    ...
    def forward(self, query, key, value, hL, hR, sharpening=False, mask=None):
        '''
            query, key, value are B*T*D
            B: is batch size
            T: is sequence length
        ...
        # change the shape for multi-head attention
        query = query.view(batch_size, -1, self.heads, self.d_k).transpose(1, 2) # [B, H, T, dk]
        key   = key.view(batch_size, -1, self.heads, self.d_k).transpose(1, 2)
        value = value.view(batch_size, -1, self.heads, self.d_k).transpose(1, 2)

        if hL is not None and hR is not None:
            _, T1, T2, _ = hL.size()
            hQ = self.fwQ(hL) # [B, T1, T2, D]
            hK = self.fwK(hR) # [B, T2, T1, D]
            hV = self.fwV(hR)

            # pdb.set_trace()

            hQ = hQ.view(batch_size, T1, T2, self.heads, self.d_k).transpose(1, 2).transpose(1, 3) # [B, H, T1, T2, dk]
            hK = hK.view(batch_size, T2, T1, self.heads, self.d_k).transpose(1, 2).transpose(1, 3) # [B, H, T2, T1, dk]
            hV = hV.view(batch_size, T2, T1, self.heads, self.d_k).transpose(1, 2).transpose(1, 3)
            ...
            # to the normal shape [batch_size, sentence_length, d_model]
            x = x.transpose(1, 2).contiguous() \
                 .view(batch_size, -1, self.heads * self.d_k) # [B, T1, D]
        else:
            x, self.attn = attention(query, key, value, mask=mask, dropout=self.dropout)
            # sharpening the attention distribution
            # if sharpening:
...
class CrEncoder(nn.Module):
    ...
    def forward(self, x, m, hL=None, hR=None, qmask=None, amask=None, sharpening=False):
        "Pass the input (and mask) through each layer in turn."
        for layer in self.layers:
            x = layer(x, m, hL, hR, qmask, amask, sharpening)
        return x
...
class CrEncoderLayer(nn.Module):
    ...
    def forward(self, x, m, hL=None, hR=None, qmask=None, amask=None, sharpening=False):
        ...
        # cross attention
        res = self.norm_in(x)
        res = self.cross_attn(res, m, m, hL, hR, sharpening, amask)
        res = self.dropout(res)
        x = x + res
...
class KNLIresnet(nn.Module):
    ...
    def __init__(self, hidden_size = 512, drop_rate = 0.1,
        ...
        '''
        super(KNLIresnet, self).__init__()
        c = copy.deepcopy
        ...
        cr_attn = ConceptMultiHeadAttn(heads=heads, d_model=embd_dim, dropout=drop_rate)
        ...
        self.cr_encoder_q = CrEncoder(layer = CrEncoderLayer(size=embd_dim, self_attn=c(attn),
                                                cross_attn=c(cr_attn), feed_forward=c(ff), dropout=drop_rate),
                                      N=num_layers_cross)
        self.cr_encoder_a = c(self.cr_encoder_q)
    ...
    def forward(self, question, answer, qmask=None, amask=None, 
        ...
        # cross attention
        question = self.cr_encoder_q(question, answer, hL, hR, qmask, amask, sharpening)
        answer = self.cr_encoder_a(answer, question, hR, hL, amask, qmask, sharpening)
